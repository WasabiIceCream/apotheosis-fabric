...
/**
 * Port of NeoForge's {@code EquippedItemTrigger} — no NeoForge coupling upstream either, ported
 * unchanged. Fired by {@code mixin.LivingEntityEquipmentTriggerMixin} on the server whenever a
 * {@link ServerPlayer}'s equipment actually changes (no NeoForge {@code LivingEquipmentChangeEvent}
 * equivalent on Fabric, so this is picked up via a mixin into vanilla's own per-tick equipment-
