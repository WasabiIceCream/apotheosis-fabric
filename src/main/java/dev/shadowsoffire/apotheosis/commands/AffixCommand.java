...
/**
 * Port note (NeoForge/Apothic-Attributes -> Fabric): {@code ApothicAttributes.getTooltipFlag()}
 * is replaced by our own {@link AttributeTooltipContext#getTooltipFlag()} (see that class's
 * javadoc). {@code AttributeHelper.list()} (an Apothic-Attributes bullet-point component
 * builder, out of scope) is replaced with a plain {@code "- "} literal prefix.
 */
public class AffixCommand {
    ...
    public static int listAffixes(CommandContext<CommandSourceStack> c) {
        ...
        if (entity instanceof LivingEntity living) {
            ...
            Map<DynamicHolder<Affix>, AffixInstance> affixes = AffixHelper.getAffixes(held);
            AttributeTooltipContext ctx = AttributeTooltipContext.of(living instanceof Player p ? p : null, TooltipContext.of(c.getSource().getLevel()), TooltipDisplay.DEFAULT, AttributeTooltipContext.getTooltipFlag());
    ...
    public static int listAlternatives(CommandContext<CommandSourceStack> c, Identifier affixId) throws CommandSyntaxException {
        ...
        if (entity instanceof LivingEntity living) {
            ...
            alternatives.forEach(a -> {
                ...
                c.getSource().sendSystemMessage(Component.literal("- ").append(name));
            });
