...
/**
 * Port note (NeoForge -> Fabric): dropped {@code attemptRadialMining(BreakBlockEvent, RadialData)}
 * — NeoForge's {@code BreakBlockEvent} has no Fabric equivalent (block-break interception needs
 * a mixin or `PlayerBlockBreakEvents`, per the plan's research — TODO once
 * {@code RadialAffix}'s dropped {@code onBreak} hook is revisited). {@link #breakExtraBlocks}
 * (the real "break blocks in a radius" logic, called by whatever hook eventually replaces
 * {@code onBreak}) is real and ported unchanged. {@link #toggleRadialState} drops NeoForge's
 * {@code PacketDistributor.sendToPlayer}/{@code RadialStatePayload} in favor of
 * {@link RadialMiningComponent}'s built-in {@code AutoSyncedComponent} sync.
 */
public class RadialUtil {
    ...
    public static void toggleRadialState(Player player) {
        ...
        RadialMiningComponent.KEY.sync(player);
    }
    ...
    public static enum RadialState {
        ...
        public static RadialState getState(Player player) {
            return RadialMiningComponent.get(player).getState();
        }

        public static void setState(Player player, RadialState state) {
            RadialMiningComponent.get(player).setState(state);
        }
    }
