...
public class GemCaseScreen extends AdventureContainerScreen<GemCaseMenu> implements DrawsOnLeft {
    ...
    public GemCaseScreen(GemCaseMenu container, Inventory inv, Component title) {
        // Port note: imageHeight is `final` on this MC version's AbstractContainerScreen, so it must
        // be passed through the super constructor instead of assigned afterward (176 is vanilla's
        // default imageWidth, unchanged from upstream).
    ...
    protected void init() {
        super.init();
        this.filter = this.addRenderableWidget(new EditBox(this.getFont(), this.leftPos + 16, this.topPos + 16, 110, 11, this.filter, Component.literal("")));
        ...
        for (int i = 0; i < MAX_ROWS * SLOTS_PER_ROW; i++) {
            var btn = new GemCaseSelectButton(this, i, this.leftPos + 21 + i % SLOTS_PER_ROW * 18, this.topPos + 31 + i / SLOTS_PER_ROW * 19);
        ...
        for (int i = 1; i < purities.length; i++) {
            ...
            var btn = SimpleTexButton.builder()
                ...
                .texSize(307, 256)
                .texPos(291, 29)
                .pos(this.leftPos + 30 + (i - 1) * 18, this.topPos + 109)
    ...
    public boolean keyPressed(KeyEvent event) {
        // Port note: this MC version's KeyMapping has no isActiveAndMatches(Key) — match the
        // KeyEvent directly against the mapping instead.
        if (this.minecraft.options.keyInventory.matches(event) && this.getFocused() == this.filter) {
            return true;
        }
        ...
        if (this.getSelectedGem() != null) {
            for (Purity p : Purity.ALL_PURITIES) {
                ...
                if (count == 0) {
                    ...
                    int x = this.leftPos + 21 + slotIndex * 18;
                    int y = this.topPos + 91;
                    if (this.isHovering(x - this.leftPos, y - this.topPos, 16, 16, mouseX, mouseY)
                        ...
                        gfx.setComponentTooltipForNextFrame(this.getFont(), tooltip, mouseX, mouseY);
                    }
                }
            }
        }
    ...
    // Port note: upstream overrides `renderSlotContents(GuiGraphicsExtractor, ItemStack, Slot, String)`,
    // a hook that no longer exists on this MC version's AbstractContainerScreen — the per-slot render
    // path was collapsed into `extractSlot(GuiGraphicsExtractor, Slot, int, int)` with no separate
    // "just draw the stack" seam. Overriding extractSlot itself instead: GemCaseSlots get the custom
    // fake-item + count-overlay render, everything else falls through to the vanilla implementation.
    @Override
    protected void extractSlot(GuiGraphicsExtractor gfx, Slot slot, int mouseX, int mouseY) {
        if (slot instanceof GemCaseSlot gss) {
            gfx.fakeItem(slot.getItem(), slot.x, slot.y);
            ...
            if (count > 1) {
                ...
                float textX = (slot.x + 16 - (this.getFont().width(countStr) - 1) * scale) / scale;
                float textY = (slot.y + 16 - (this.getFont().lineHeight - 2) * scale) / scale;
                gfx.text(this.getFont(), countStr, (int) textX, (int) textY, 0xFFFFFFFF, true);
                gfx.pose().popMatrix();
            }
        }
        else {
    ...
    private OnPress tryUpgrade(Purity purity) {
        return btn -> {
            ...
            ClientPlayNetworking.send(new dev.shadowsoffire.placebo.payloads.ButtonClickPayload(value));
        };
    }
