...
/**
 * Port note (NeoForge -> Fabric): drops {@code Apoth.Triggers.GEM_CUTTING} advancement-trigger
 * firing (advancements are out of scope for this port — see README) and the NeoForge
 * {@code transfer} API's per-index {@code getCapacity} override on the base slot; the base
 * slot's 1-item cap is instead enforced with an anonymous {@link PlaceboContainerMenu.UpdatingSlot}
 * override of {@code getMaxStackSize()}.
 */
public class GemCuttingMenu extends PlaceboContainerMenu {
    ...
    public static List<RecipeHolder<GemCuttingRecipe>> getRecipes(Level level) {
        ...
        return level.getServer().getRecipeManager().getRecipes().stream()
            .filter(holder -> holder.value() instanceof GemCuttingRecipe)
            .map(holder -> (RecipeHolder<GemCuttingRecipe>) (RecipeHolder<?>) holder)
            .toList();
    }
}
