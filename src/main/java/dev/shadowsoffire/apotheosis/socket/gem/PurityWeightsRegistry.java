...
public final class PurityWeightsRegistry extends DynamicRegistry<PurityWeights> {
    ...
    // No longer resets parsedWeights in beginReload(); rebuilds off to the side and atomically
    // swaps at the end of onReload() instead, so a concurrent getWeights() read never sees the
    // ERRORED (cracked-only) fallback mid-reload just because the map happened to be empty at
    // that instant.
    @Override
