...
public class ExtraGemBonusRegistry extends DynamicRegistry<ExtraGemBonus> {
    ...
    // No longer resets extraBonuses in beginReload(); rebuilds off to the side and atomically
    // swaps at the end of onReload() instead, so a concurrent getBonusesFor() read never sees an
    // empty/partially-rebuilt map mid-reload.
    @Override
    protected void onReload(ReloadType type) {
        super.onReload(type);
        Multimap<DynamicHolder<Gem>, ExtraGemBonus> rebuilt = HashMultimap.create();
        for (ExtraGemBonus extraBonus : this.getValues()) {
            rebuilt.put(extraBonus.gem, extraBonus);
        }
        this.extraBonuses = rebuilt;
    }
