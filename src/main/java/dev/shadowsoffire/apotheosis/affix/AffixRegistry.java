...
public class AffixRegistry extends TieredDynamicRegistry<Affix> {
    ...
    // Port note: found by actually launching a dev server. Affix entries reference
    // DynamicHolder<LootRarity> in their "values" maps, decoded via RarityRegistry's
    // holderCodec() — but DynamicRegistry's default getFabricDependencies() only depends on
    // DynamicTagManager, not on other apotheosis registries, so Fabric's reload scheduler had no
    // guarantee RarityRegistry finished applying (and bound its holders) before this one ran.
    // Without this, every affix file failed with "Trying to access unbound value" NullPointerExceptions.
    @Override
    ...
    // Port note: found live — this used to clear `byType` to empty in beginReload(), well before
    // onReload() rebuilds it at the end of the same reload. Any concurrent read of getTypeMap()
    // during that whole window (e.g. natural loot generation running on another thread, such as
    // Lootr's per-player container loot generation) saw an empty pool and failed with
    // "AffixLootRule (no affixes available)" — a real race, not a data gap, confirmed by ~75
    // fresh loot rolls immediately after producing zero repeats. Fix: don't clear byType early;
    // leave the old (stale but valid) map in place until onReload() atomically replaces it with
    // the new one. Readers mid-reload now see slightly-stale affix data instead of none.
    @Override
    protected void onReload(ReloadType type) {
        ...
        // Port note: dropped the original's dev-only client lang-key validation
        // (AdventureModuleClient.checkAffixLangKeys()) — that client debug utility isn't
        // ported yet. Not load-bearing for correctness, just a missing-translation warning.
