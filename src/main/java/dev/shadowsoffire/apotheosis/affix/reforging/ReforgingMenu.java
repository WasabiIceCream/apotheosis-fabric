...
/**
 * Port note (NeoForge -> Fabric): {@code ReforgingResultSlot} extended NeoForge's
 * {@code ResourceHandlerSlot} for its ability to bypass {@code Slot}'s vanilla
 * {@code Container}-backed constructor; here it extends {@link Slot} directly, backed by
 * {@code choicesInv.asContainer()} — same as every other custom output slot ported so far (see
 * {@code socket.gem.storage.GemCaseSlot}). {@code player.getPersistentData()} is replaced with
 * {@link PersistentDataComponent} — see that class's javadoc.
 */
public class ReforgingMenu extends BlockEntityMenu<ReforgingTableTile> {
    ...
    public ReforgingMenu(int id, Inventory inv, BlockPos pos) {
        ...
        this.addSlot(new UpdatingSlot(this.tile.getInventory(), 0, 39, 40, this.tile::isValidRarityMat));
        this.addSlot(new UpdatingSlot(this.tile.getInventory(), 1, 123, 86, stack -> stack.is(Items.SIGIL_OF_REBIRTH)));
    ...
    protected void updateSeed() {
        int seed = PersistentDataComponent.get(this.player).getIntOr(REFORGE_SEED, 0);
        if (seed == 0) {
            seed = this.player.getRandom().nextInt();
            PersistentDataComponent.get(this.player).putInt(REFORGE_SEED, seed);
        }
        this.seed = seed;
    }
    ...
    public class ReforgingResultSlot extends Slot {
        ...
        public ReforgingResultSlot(InternalItemHandler itemHandler, int index, int xPosition, int yPosition) {
            super(itemHandler.asContainer(), index, xPosition, yPosition);
        }
        ...
        public boolean mayPickup(Player playerIn) {
            ...
            ReforgingRecipe recipe = rarity == null ? null : ReforgingMenu.this.tile.getRecipeFor(rarity);
            if (recipe == null || input.isEmpty()) {
                return false;
            }

            int sigils = ReforgingMenu.this.getSigilCount();
            int sigilCost = ReforgingMenu.this.getSigilCost(this.getContainerSlot());
            int mats = ReforgingMenu.this.getMatCount();
            int matCost = ReforgingMenu.this.getMatCost(this.getContainerSlot());
            int levels = ReforgingMenu.this.player.experienceLevel;
            int levelCost = ReforgingMenu.this.getLevelCost(this.getContainerSlot());
        ...
        public void onTake(Player player, ItemStack stack) {
            if (!player.level().isClientSide()) {
                ...
                if (!player.isCreative()) {
                    ...
                    EnchantmentUtils.chargeExperience(player, ApothMiscUtil.getExpCostForSlot(levelCost, this.getContainerSlot()));
                }
                PersistentDataComponent.get(player).putInt(REFORGE_SEED, player.getRandom().nextInt());
