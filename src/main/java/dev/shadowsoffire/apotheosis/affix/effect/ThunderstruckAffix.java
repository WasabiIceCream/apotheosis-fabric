 ...
 * Damage Chain
 * <p>
 * Port note: see {@code Apoth.DamageTypes.THUNDERSTRUCK}'s javadoc — the damage source used
 * here is a real, statically-tagged damage type instead of upstream's runtime tag mutation.
 */
public class ThunderstruckAffix extends Affix {
    ...
    public void doPostAttack(AffixInstance inst, LivingEntity user, Entity target) {
        ...
        if (ApothMiscUtil.getLocalAtkStrength(user) >= 0.98) {
            ...
            for (Entity e : nearby) {
                DamageSource src = user.damageSources().source(Apoth.DamageTypes.THUNDERSTRUCK, user);
