...
public class DamageReductionAffix extends Affix {
    ...
    /**
     * Port note: {@code isPhysicalDamage} replaces Apothic-Attributes' own
     * {@code AttributesUtil.isPhysicalDamage} (out of scope as a dependency) — a reasonable
     * approximation (a directly-entity-caused hit not otherwise classified as explosion, fire,
     * fall, lightning, or magic), used only for this affix's own damage-type bucketing, not any
     * gameplay-critical logic.
     */
    private static boolean isPhysicalDamage(DamageSource d) {
        return d.getEntity() != null && !d.is(DamageTypeTags.IS_EXPLOSION) && !d.is(DamageTypeTags.IS_FIRE)
            && !d.is(DamageTypeTags.IS_FALL) && !d.is(DamageTypeTags.IS_LIGHTNING) && !d.is(Apoth.Tags.IS_MAGIC);
    }

    public static enum DamageType implements Predicate<DamageSource>, StringRepresentable {
        PHYSICAL("physical", DamageReductionAffix::isPhysicalDamage),
        MAGIC("magic", d -> d.is(Apoth.Tags.IS_MAGIC)),
